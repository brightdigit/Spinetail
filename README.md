...
Marketing API. It is intentionally small: it wraps two generated operations —
**listing sent campaigns** and **fetching a campaign's content** — and exposes
a thin public surface for its consumer
([`ContributeMailchimp`](https://github.com/brightdigit), a newsletter
...
`campaignContent(forCampaignID:)` and `plainText(forCampaignID:)` are convenience
methods over the same generated `getCampaignsIdContent` operation —
...
let plain = try await client.plainText(forCampaignID: "campaign-id")
```

## Error handling

Invalid HTTP responses and missing content fields are mapped to
...
| `.missingPlainText(campaignID:)` | The campaign returned no `plain_text`.                   |
